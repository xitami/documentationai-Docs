...
**Leseliste**-Plugin, das auf Lesbarem die Aktion „Zur Leseliste" anbietet und den
Inhalt als eigenen Eintrag im Content-Store ablegt:

```typescript
const readlistPlugin: DeclarativePlugin = {
  id: "de.brainz.readlist",
  name: "Leseliste",
  description: "Fügt die Aktion Zur Leseliste hinzu — legt Inhalte als eigene Einträge zum späteren Lesen an.",
  icon: "book",
    ...
    actions: [
      {
        ...
        contentTypes: ["bookmark", "article", "link", "blogpost", "story", "document"],
        effect: {
          type: "createEntry",
          contentType: "readlist_item",
          title: "title",          // gepunkteter Feldpfad → wird der Titel
          from: { url: "url", source: "domain" },  // weitere Felder ins json_data
        },
      },
    ],
  },
...
| `createEntry` | `contentType`, `title?`, `from?` | Legt einen Eintrag im **Content-Store** an (`POST /api/drop`) |

### Persistenz: der Content-Store, kein Plugin-Silo

`createEntry` schreibt über `POST /api/drop` einen **ganz normalen Eintrag** — denselben
Speicher, den auch der Rest der App nutzt. `sourceUuid` verlinkt automatisch zurück auf
die Quelle. Das ist bewusst so (Entscheidung 3 der Architektur): Plugin-Daten leben im
Dokumenten-/Content-Store, nicht in einem separaten Plugin-Speicher.
...
- **Leseliste** (deklarativ) — „Zur Leseliste" legt ein `readlist_item` im Content-Store an.
