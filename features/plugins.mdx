...
**Status (Stand 15.06.2026).** Das Plugin-System wird in der neuen App
(app.brainz.plus/app) frisch aufgebaut — die kanonische Architektur steht in
`specs/plugin-architecture.md`. **Heute verfügbar (P1): deklarative Plugins** — reine
Daten-Manifeste ohne eigenen Code (kein Sandbox-Risiko), Persistenz über den
**Content-Store**. **Geplant:** P2 Code-Plugins im Web-Worker-Sandbox, P3 öffentliche
Registry + Veröffentlichung. Die Abschnitte ab „Plugin-Typen" beschreiben die
**Ziel-Architektur** — Teile davon sind noch nicht live. Im Zweifel gilt der
**Schnellstart** direkt hier drunter.

</Callout>

## Schnellstart: Dein erstes Plugin (deklarativ, P1 — heute verfügbar)

Ein **deklaratives Plugin ist reine Daten** — ein Manifest, kein eigener Code. Der Host
mappt es in die bestehenden Action-Slots der App. Dadurch gibt es kein Sandbox-Risiko,
und der An/Aus-Schalter wirkt sofort ohne Reload. Das ist der Teil des Plugin-Systems,
der heute live ist.

### Das Manifest

Ein Plugin beschreibt sich über `contributes.actions`. Beispiel — das mitgelieferte
**Leseliste**-Plugin: es markiert Artikel & Bücher als „zu lesen", indem es den
Lese-Status **am Inhalt selbst** setzt (genau wie die Mediathek) — ohne eine Kopie
anzulegen:

```typescript
const readlistPlugin: DeclarativePlugin = {
  id: "de.brainz.readlist",
  name: "Leseliste",
  description: "Markiert Artikel & Bücher als Leseliste — setzt den Lese-Status am Inhalt selbst (Mediathek), ohne Kopie.",
  icon: "book",
    ...
    actions: [
      {
        ...
        label: "Zur Leseliste",
        icon: "book",
        surface: "bar",                     // "bar" = Aktionsleiste oben | "section" = Aktions-Sektion
        rank: 34,                           // Sortierung innerhalb des Slots
        contentTypes: ["article", "book"],  // Parität zur System-Collection „Leseliste"
        effect: { type: "setStatus", status: "want" },  // Status am Inhalt (data.media_status)
      },
    ],
  },
};
...
Registriert wird es mit `registerDeclarativePlugin(readlistPlugin)`. Die App führt das
beim Start für alle Plugins aus; sichtbar werden Aktionen erst, wenn das Plugin
eingeschaltet ist (siehe Aktivieren).

### Sichtbarkeit: zwei Bedingungen

Eine Plugin-Aktion erscheint nur, wenn **beide** zutreffen:

1. **Das Plugin ist aktiviert** (Toggle in den Einstellungen).
2. **Das deklarative `appliesWhen` trifft zu** — so frustriert keine Aktion, die nichts tut.

| `appliesWhen`-Form | Bedeutung |
| --- | --- |
| weggelassen / `always: true` | immer (sofern Plugin an und `contentTypes` passt) |
| `contentType: ["bookmark", …]` | nur bei diesen Content-Typen |
| `hasField: "data.url"` | nur wenn dieses Feld einen Wert hat |

Feldpfade sind **gepunktet und JSON-String-sicher**: `structured_data.url` greift auch
dann, wenn `structured_data` als JSON-String vorliegt — die Zwischenebenen werden
automatisch ausgepackt.

### Effekte (fester, sicherer Satz)

Ein deklaratives Plugin wählt aus einem festen Satz von Effekten. Kein beliebiger Code:

| Effekt | Felder | Wirkung |
| --- | --- | --- |
| `openUrl` | `from` | Öffnet die URL (nur `http`/`https`) in neuem Tab; sonst Hinweis „Keine URL vorhanden" |
| `copy` | `fields` | Kopiert die verketteten Feldwerte in die Zwischenablage |
| `share` | `title?`, `text?` | Teilen-Dialog (Web Share API), Fallback Zwischenablage |
| `createEntry` | `contentType`, `title?`, `from?` | Legt einen **neuen** Eintrag im Content-Store an (`POST /api/drop`) — für noch nicht erfasste Inhalte/Exporte |
| `setStatus` | `status` | Setzt `data.media_status` **am geöffneten Inhalt** (`POST /api/groups/mediathek/status`) — kein neuer Eintrag |

### Persistenz: der Content-Store, kein Plugin-Silo

Plugin-Daten leben im bestehenden Dokumenten-/Content-Store, nicht in einem separaten
Plugin-Speicher (Entscheidung 3 der Architektur). Zwei Muster, je nachdem ob der Inhalt
schon existiert:

- **Marker am bestehenden Inhalt** (`setStatus`) — bevorzugt. Es wird **kein** Duplikat
  angelegt, sondern ein Status-Feld am Inhalt selbst gesetzt (`data.media_status`), genau
  wie die Mediathek „want / active / done" führt. Die System-Smart-Collection „Leseliste"
  (book/article) zeigt den Inhalt dann automatisch an.
- **Neuer Eintrag** (`createEntry`) — nur für Inhalte, die es noch nicht gibt
  (Erst-Erfassung) oder echte Exporte. Schreibt über `POST /api/drop`; `sourceUuid`
  verlinkt zurück auf die Quelle.

### Aktivieren

**Einstellungen → Darstellung → Erweiterungen** → Plugin einschalten. Wirkt sofort,
ohne Reload. Zwei Plugins sind bereits live:

- **Leseliste** (deklarativ) — „Zur Leseliste" setzt auf Artikeln & Büchern den Lese-Status am Inhalt selbst (Mediathek „want"), ohne Kopie.
- **Bring! Einkaufsliste** (Code-Capability) — „An Bring! senden" teilt Zutaten/Listen an die Bring!-App.

> Code-Capabilities wie Bring! sind kuratiert mitgeliefert. Fremder Plugin-Code im
> Sandbox (P2) und eine öffentliche Registry (P3) folgen — siehe Ziel-Architektur unten.
...
**Frontend-Erweiterungen werden als [Capabilities](/features/capabilities) gebaut** — dem descriptor-nativen Weg, einen `typeView` oder `renderer` zu realisieren. Diese Seite beschreibt die *Plattform*: Manifest, Trust-Level, Sandbox, Marketplace und Lifecycle. *Wie* ein Frontend-Beitrag konkret rendert, steht unter [Capabilities](/features/capabilities). Die frühere imperative API (`brainz.ui.registerTypeView` mit direktem DOM) ist damit Legacy.
...
| `homeSection` | Deklarative Sektion im Adaptiven Home — modus-gegated, rendert über einen render_ui-Descriptor |

In der descriptor-basierten App werden diese Frontend-Typen als [Capabilities](/features/capabilities) realisiert: `typeView` entspricht den `renderers`- und `detail`-Slices einer Capability, `quickAction` und `shortcut` den Action-Slices. Eine Capability bündelt sie pro Kind — Content-Typ oder Knowledge Entity — an einer Stelle.

#### Home-Sektionen (`homeSection`)

Das **Adaptive Home** ist ein Komponist: eine Context-Engine wählt aus Signalen (Tageszeit · Wochentag · Linse · Flow) einen *Modus* und komponiert daraus eine geordnete Liste von **Sektionen**. Sektionen sind **deklarativ** — ein Plugin beschreibt, *wann* eine Sektion erscheint und *was* sie zeigt, nicht *wie* sie gezeichnet wird.

**Sektion-Vertrag** (`HomeSectionDescriptor`):

| Feld | Zweck |
| --- | --- |
| `id` | Stabile Kennung + Position im Composer-Feed |
| `appliesWhen(signals)` | Prädikat über dem Signal-Snapshot — entscheidet, ob die Sektion im aktuellen Modus zählt |
| `isEmpty(ctx)` | Leer ⇒ die Sektion fällt weg (nie ein leeres Blatt) |
| `render(ctx)` | Liefert einen **render_ui-Descriptor** (Katalog B) |

**Bausteine (live, Katalog B):** Sektionen rendern über drei render_ui-Komponenten, die im [Design-System](https://app.brainz.plus/app/design-system/) dokumentiert sind:

| Komponente | Zweck |
| --- | --- |
| `AdaptiveSection` | Getitelter Rahmen (z. B. „JETZT", „Heute Abend"); Kinder = Inhalt |
| `Shelf` | Horizontale, snap-scrollbare Karten-Schiene (z. B. „Filmabend") |
| `DayTimeline` | Getypte Tagesagenda (Termine/Erinnerungen) |

Weil eine Sektion über einen render_ui-Descriptor rendert, erbt sie automatisch Theming, Dark Mode und die geteilten Karten (z. B. `MediaCard`).

**Eigene Sektionen beisteuern.** Eine Capability liefert `homeSections[]` — jeder Beitrag bestimmt selbst, *wann* er erscheint (`appliesWhen` über dem Signal-Snapshot), ob er bei leerer Datenlage wegfällt (`isEmpty`), *was* er rendert (`render` → render_ui-Descriptor) und *wo* (`placement: "top" | "bottom"`, Default `bottom`):
...
registerCapability({
  ...
  homeSections: [{
    ...
    title: "Wetter",
    placement: "top",
    appliesWhen: ({ snapshot }) => snapshot.signals.hasGeo,
    isEmpty: () => false,
    render: () => ({ type: "WeatherStrip", props: { /* … */ } }),
  }],
});
```

Der Home-Composer mischt diese Beiträge gegated zu den Built-in-Sektionen (`top` davor, `bottom` danach). So erweitern Plugins das Home, ohne den Core anzufassen.

**Data-only (deklaratives Manifest).** Ein Plugin **ohne Code** beschreibt eine Home-Sektion rein als Daten: ein **statischer render_ui-Descriptor** (`$pfad`-Strings werden gegen den Home-Snapshot aufgelöst, z. B. `"$composition.mode"`) plus ein `appliesWhen` über dem Snapshot:
...
```json
{
  "contributes": {
    "homeSections": [{
      ...
      "title": "Filmabend",
      "placement": "top",
      "appliesWhen": { "hasField": "weekend" },
      "render": {
        "type": "AdaptiveSection",
        "props": { "title": "Filmabend" },
        "children": [{ "type": "BodyText", "props": { "text": "Wochenende — Zeit für einen Filmabend?" } }]
      }
    }]
  }
}
...
Der Host rendert nur **bekannte Komponenten** (die die Home-Engine kennt: `AdaptiveSection` · `Shelf` · `DayTimeline` · `BodyText` · `MediaCard` …) — kein Plugin-Code, keine Sandbox. Eine **Persona** ist genau ein Bündel solcher deklarativer Sektionen (Beispiel-Plugin „Persona: Filmfan").

<Note>
Status: render_ui-Bausteine, der Sektion-Vertrag, die Code-Capability-Verdrahtung (`collectHomeSections` → Composer) UND der **deklarative, data-only Pfad** (`DeclarativePlugin.contributes.homeSections`, `isPluginEnabled`-gated) sind **live**. Offen: ein Sandbox-Thunk für Sektionen mit echter Laufzeit-Logik (P2+).
</Note>
...
Legacy: registriert einen TypeView imperativ über DOM. In v2 wird ein TypeView als [Capability](/features/capabilities) geliefert.
...
#### TypeView als Capability

Ein Frontend-Plugin liefert seinen `typeView` als **Capability** statt über imperatives DOM. Der Frontend-Entrypoint stellt die Capability bereit; der Host registriert sie (trusted: direkt; community: gesandboxt). Statt `renderStreamCard`/`renderFullView` zu bauen, liefert sie **Descriptor-Renderer** und eine **Detail-Komposition**:

```ts
export const invoiceCapability: Capability = {
  id: "invoice",
  renderers: { InvoiceCard: invoiceCardRenderer },          // Descriptor → DOM
  detail: { contentTypes: ["invoice"], compose: composeInvoice },
};
```

Den vollständigen Vertrag — Sinks, Identität, Intent, Actions und Knowledge-Entity-Beziehungen — sowie ein lauffähiges Beispiel beschreibt die Seite [Capabilities](/features/capabilities).

<Callout kind="info">

Die frühere imperative API `brainz.ui.registerTypeView` (DOM aus `renderStreamCard`/`renderFullView`) ist Legacy. Quick Actions, Commands und UI-Helfer (`toast`, `haptic`, `openDetail`) bleiben über `brainz.ui` verfügbar.
...
Community-Plugins laufen in einem dedizierten Web Worker. Dieser Worker hat keinen Zugriff auf `window`, `document` oder andere DOM-APIs, sondern kommuniziert ausschließlich über `postMessage` mit dem Main Thread. Genau dazu passt das Capability-Modell: Ein gesandboxtes Frontend-Plugin liefert **Descriptors** (Komponenten-Bäume), die der Host rendert — niemals direktes DOM. Die DOM-Freiheit der Sandbox ist damit keine Einschränkung, sondern die natürliche Grenze.
